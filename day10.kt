...
fun createMapFromFile(fileName: String): Array<Array<Int>> {
...
fun searchPath(map: Array<Array<Int>>, y: Int, x: Int) {
    if(map[y][x] == 9) {
        rating++
        trails.add(Pair(y, x))
        //println("Haha (${y},${x})")
    ...
    if(y + 1 < MAP_SIZE && map[y + 1][x] == (map[y][x] + 1)) {
        searchPath(map, y + 1, x)
    } 
    if(y - 1 >= 0 &&  map[y - 1][x] == (map[y][x] + 1)) {
        searchPath(map, y - 1, x)
    } 
    if(x + 1 < MAP_SIZE &&  map[y][x + 1] == (map[y][x] + 1)) {
